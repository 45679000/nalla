	...
	$credit_limit, $sales_type, $notes)
{
	...
	try{
		$sql = "INSERT INTO  ".$this->tbpref."debtors_master (name, debtor_ref, address, tax_id,
...
{
	try{
		$sql = "UPDATE ".$this->tbpref."debtors_master SET name= ?, 
...
function delete_customer(){
	try{
		$sql = "DELETE FROM ".$this->tbpref."debtors_master WHERE debtor_ref= ?";
		$stmt = $this->conn->prepare($sql);
...
function get_customer($customer_id){
	$sql = "SELECT * FROM  ".$this->tbpref."debtors_master WHERE debtor_ref= ?";
	$customer=array();
	 $stmt = $this->conn->prepare($sql);
...
//    
//
//  //  $ref = str_replace(' ', '-', $ref); // Replaces all spaces with hyphens.
//
//   $ref = preg_replace('/[^A-Za-z0-9\-]/', '', $ref); // Removes special chars.

    
	try {
		
           // $sql = "SELECT * FROM  ".$this->tbpref."debtors_master WHERE debtor_ref= ? or REPLACE(debtor_ref, ' ', '')= ? or REPLACE(name, ' ', '')= ?";
		$sql = "SELECT * FROM  ".$this->tbpref."debtors_master WHERE debtor_no= ?";
		$stmt = $this->conn->prepare($sql);
