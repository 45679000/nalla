...
class Item{
	...
	try{
		$sql = "INSERT INTO `".$this->tbpref."item_codes`(`item_code`, `stock_id`, `description`, `category_id`, `quantity`, `is_foreign`)
         VALUES (?,?,?,?,?,?)";
...
function add_stock_item($item_code, $stock_id, $description, $category, $qty, $foreign=0){
	...
	try{
		...
		$sql2 = "INSERT INTO ".$this->tbpref."stock_master (stock_id, description, long_description, category_id,
...
function update_item_code($item_code, $stock_id, $description, $category, $qty, $foreign=0){
	try{
		$sql = "UPDATE ".$this->tbpref."item_codes SET item_code =?, stock_id = ?, description = ?,
...
function delete_item_code(){
	try{
		$sql = "DELETE FROM ".$this->tbpref."item_codes WHERE item_code= ?";
		$stmt = $this->conn->prepare($sql);
...
function get_item_code($id){
	...
	$sql = "SELECT * FROM  ".$this->tbpref."item_codes WHERE item_code= ?";
	$items=array();
	 $stmt = $this->conn->prepare($sql);
     $stmt->bindParam(1, $id);
        // execute query
...
function get_item_price($id){
	$sql = "SELECT * FROM  ".$this->tbpref."prices WHERE stock_id= ?";
	$price=array();
	 $stmt = $this->conn->prepare($sql);
     $stmt->bindParam(1, $id);
        // execute query
...
function add_item_price($stock_id, $sales_type_id, $curr_abrev, $price){
	try{
		$sql = "INSERT INTO `".$this->tbpref."prices`(`stock_id`, `sales_type_id`, `curr_abrev`, `price`)
        VALUES (?,?,?,?)";
			$stmt = $this->conn->prepare($sql);
...
function update_item_price($stock_id, $sales_type_id, $curr_abrev, $price){
	try{
		$sql = "UPDATE ".$this->tbpref."prices SET sales_type_id = ?, curr_abrev = ?,
	 	price = ?
		WHERE stock_id = ?";
		$stmt = $this->conn->prepare($sql);
...
function delete_item_price($stock_id){
	try{
		$sql = "DELETE FROM ".$this->tbpref."prices WHERE id= ?";
		$stmt = $this->conn->prepare($sql);
		$stmt->bindParam(1, $stock_id);
		$stmt->execute();
