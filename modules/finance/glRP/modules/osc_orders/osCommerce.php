...
if (isset($_POST['action'])) {
    ...
    if ($action == 'update') {
        if (isset($_POST['dbHost'])) $connost = $_POST['dbHost'];
        ...
        if ($connost != $db_Host) { // It changd
            if ($connost == '') $sql = "DELETE FROM ".TB_PREF."oscommerce WHERE name = 'myhost'";
            else if ($db_Host == '') $sql = "INSERT INTO ".TB_PREF."oscommerce (name, value) VALUES ('myhost', '" . $connost . "')";
	    else $sql = "UPDATE  ".TB_PREF."oscommerce SET value = '" . $connost . "' WHERE name = 'myhost'";
    ...
    if ($action == 'c_import') {
        ...
        else $osc = mysqli_connect($connost, $dbUser, $dbPassword);
...
if ($action == 'show') {
	...
	if ($found) {
    	text_row("Mysql Host", 'dbHost', $connost, 20, 40);
