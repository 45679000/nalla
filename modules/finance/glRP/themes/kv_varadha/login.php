...
function defaultCompany()
{
	...
	echo "<link href='$path_to_root/themes/kv_varadha/default.css' rel='stylesheet' type='text/css'> \n";
	...
	if (!$login_timeout) { // FA logo
    	echo "<a target='_blank' href='$power_url'><img src='$path_to_root/themes/$def_theme/images/logo_frontaccounting.png' alt='Savannah ERP' height='50' onload='fixPNG(this)' border='0' /></a>";
	} else { 
	...
	text_row(_("User name"), "user_name_entry_field", "admin", 300, 30);

	// $password = $allow_demo_mode ? "password":"";

	password_row(_("Password:"), 'password', "admin123#");
	...
	if ($login_timeout) {
		...
		if (!@$text_company_selection) {
			echo "<tr><td>"._("Organisation")."</td><td><select name='company_login_name'>\n";
...
//			$coy = $def_coy;
			text_row(_("Organisation"), "company_login_nickname", "", 20, 50);
		}
		start_row();
	...
	start_row();
		// echo "<td colspan='2'  align='center' style='paddin-top: 10px;paddin-bottom: 10px' ><input type='submit' class='buttons' value='&nbsp;&nbsp;"._("Login &rarr;")."&nbsp;&nbsp;' name='SubmitUser'"	.($login_timeout ? '':" onclick='set_fullmode();'").(isset($blocked_msg) ? " disabled" : '')." /> </td>";
		end_row();
	end_table(1);
